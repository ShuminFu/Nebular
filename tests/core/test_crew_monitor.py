...
class TestCrewMonitor:
    ...
    @pytest.mark.asyncio
    async def test_check_new_bots(self, monitor):
        """测试检查新Bot功能"""
        # 增加BOT_NAME_FILTER常量的模拟，确保与代码匹配
        with mock.patch("src.core.entrypoints.crew_manager_main.BOT_NAME_FILTER", "前端-"):
            # 预设已管理的Bot
            ...
            # 模拟parser.parse_response返回值 - 包含新的符合条件的Bot
            monitor.parser.parse_response.return_value = (
                200,
                [
                    {"id": "bot1", "name": "前端-测试Bot1", "isActive": False},  # 已管理
                    {"id": "bot2", "name": "前端-测试Bot2", "isActive": False},  # 已管理
    ...
    async def test_periodic_check(self, monitor):
        """测试定期检查功能"""
        # 模拟_check_new_bots方法
        with mock.patch.object(monitor, "_check_new_bots") as mock_check:
            ...
            with mock.patch("asyncio.sleep", side_effect=[None, Exception("Stop test")]):
                ...
                # 验证调用了检查方法(由于循环会在第二次sleep时抛出异常,所以会调用两次check_new_bots)
