...
def sample_crew_config():
    return {
        "agents": {
            "code_generator": {
                "role": "injected code_generator, 专业的代码生成和优化专家",
                "goal": "injected生成高质量、可维护的代码",
                "backstory": """injected作为一名经验丰富的代码生成专家，我精通各种编程语言和框架。
                我注重代码的可读性、可维护性和性能，能够根据需求生成最佳实践的代码实现。
                我会考虑项目的整体架构，确保生成的代码符合项目规范和最佳实践。""",
            }
        },
        "tasks": {
            "code_generation_task": {
                "description": """injected根据以下信息生成代码：
                1. 文件路径：{file_path}
                2. 文件类型：{file_type}
                3. 需求描述：{requirement}
                4. 项目信息：
                ...
                5. 相关文件：{resources}
                6. 引用关系：{references}""",
                "expected_output": """@file: {file_path}
...
async def test_create_dynamic_crew(mock_bot_id, mock_parent_bot_id, sample_crew_config):
    ...
    dynamic_crew = runner._create_dynamic_crew()
